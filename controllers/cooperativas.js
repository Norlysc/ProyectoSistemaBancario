...
class CooperativaController {
  AñadirCooperativa(req, res) {
    ...
    const nuevaCooperativa = {
      id: parseInt(id),
      nombre,
      usuariosDeCooperativa: usuariosDeCooperativa || [],
    };
    CooperativaModel.añadirCooperativa(nuevaCooperativa)
      .then((results) =>
        res
          .status(201)
          .json({
            mensaje: "Se ha añadido la cooperativa",
            cooperativa: nuevaCooperativa,
          })
      )
  ...
  EliminarUsuarioDeCooperativa(req, res) {
    ...
    CooperativaModel.eliminarUsuarioDeCooperativa(cooperativaId, usuarioId)
      .then((results) =>
        res.json({ mensaje: "Usuario eliminado de la cooperativa" })
      )
      .catch((error) => res.status(500).json({ error: error.message }));
  }
  ...
  RelacionarUsuarioConCooperativa(req, res) {
    ...
    CooperativaModel.relacionarUsuarioConCooperativa(cooperativaId, usuarioId)
      .then((results) =>
        res.json({ mensaje: "Usuario añadido a la cooperativa" })
      )
      .catch((error) => res.status(500).json({ error: error.message }));
  }
      ...
      .then((cooperativa) => {
        if (cooperativa) {
          ...
          res
            .status(404)
            .json({
              mensaje: "No se encontró la cooperativa en la base de datos",
            });
        }
      })
      .catch((error) => res.status(500).json({ error: error.message }));
